    ...
    try:
        ...
        for i, trans in enumerate(transactions):
            ...
            if amount is None or (
                isinstance(amount, float) and (amount != amount)
            ):  # NaN check
        ...
        print("\n📊 Analysis Results:")
        print(
            f"  Total Transactions: {analysis.get('summary', {}).get('total_transactions')}"
        )
        print(
            f"  Total Income: ${analysis.get('summary', {}).get('total_income', 0):,.2f}"
        )
        print(
            f"  Total Expenses: ${analysis.get('summary', {}).get('total_expenses', 0):,.2f}"
        )
        ...
        expense_categories = {
            k: v for k, v in categories.items() if v.get("type") == "expense"
        }
