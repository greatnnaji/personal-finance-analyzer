...
class PDFParserDebug:
    ...
    def validate_transaction(self, trans: Dict, index: int) -> List[str]:
        ...
        if "amount" not in trans:
            ...
            isinstance(trans["amount"], float) and trans["amount"] != trans["amount"]
        ):  # NaN check
    ...
    def parse_and_validate(self, pdf_path: str, save_debug=True) -> tuple:
        ...
        try:
                ...
                try:
                    ...
                    if debit != debit or credit != credit:  # NaN check
                        conversion_issues.append(
                            f"Trans {idx + 1}: NaN in debit/credit values"
                        )
                        continue
                    ...
                    if debit > 0:
                        ...
                        conversion_issues.append(
                            f"Trans {idx + 1}: No amount (debit={debit}, credit={credit})"
                        )
                        continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            date = datetime.now()
                            conversion_issues.append(
                                f"Trans {idx + 1}: Invalid date, using current date"
                            )
            ...
            validation_report["statistics"] = {
                ...
                "total_debits": sum(
                    -t["amount"] for t in standardized if t["amount"] < 0
                ),
                "total_credits": sum(
                    t["amount"] for t in standardized if t["amount"] > 0
                ),
            ...
            if save_debug:
                ...
                report_file = os.path.join(
                    self.DEBUG_DIR, "debug_validation_report.json"
                )
                ...
                trans_file = os.path.join(
                    self.DEBUG_DIR, "debug_standardized_transactions.json"
                )
...
"""
        ...
        messages = prompt.format_messages(
            text=pdf_text[:15000], format_instructions=self.format_instructions
        )
...
if __name__ == "__main__":
    ...
    if report["issues"]:
        print(
            f"⚠️  {len(report['issues'])} issues found - check utils/debug_validation_report.json"
        )
