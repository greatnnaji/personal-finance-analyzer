...
class DataAnalyzer:
    ...
    def _analyze_spending_trends(self, df: pd.DataFrame) -> Dict:
        ...
        # Daily spending
        daily_spending = (
            expenses_df.groupby(expenses_df["date"].dt.date)["amount"].sum().abs()
        )

        # Weekly spending
        weekly_spending = (
            expenses_df.groupby(expenses_df["date"].dt.isocalendar().week)["amount"]
            .sum()
            .abs()
        )
    ...
    def _analyze_income_vs_expenses(self, df: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            "average_income_per_transaction": round(
                income_transactions["amount"].mean(), 2
            )
            if not income_transactions.empty
            else 0,
            "average_expense_per_transaction": round(
                abs(expense_transactions["amount"].mean()), 2
            )
            if not expense_transactions.empty
            else 0,
            "income_to_expense_ratio": round(
                income_transactions["amount"].sum()
                / abs(expense_transactions["amount"].sum()),
                2,
            )
            if not expense_transactions.empty
            else 0,
    ...
    def _analyze_spending_patterns(self, df):
        ...
        spending_by_day = (
            df[df["amount"] < 0].groupby("day_of_week")["amount"].sum().abs()
        )
        ...
        if date_range > 0:
            patterns["average_daily_spending"] = float(
                abs(df[df["amount"] < 0]["amount"].sum()) / date_range
            )
    ...
    def _detect_spending_anomalies(self, df, analysis):
        ...
        if len(monthly_data) >= 2:
            ...
            avg_previous_spending = np.mean(
                [
                    abs(monthly_data[month]["total_expenses"])
            ...
            if avg_previous_spending > 0:
                change_percent = (
                    (current_spending - avg_previous_spending) / avg_previous_spending
                ) * 100
        ...
        if len(current_month_data) > 0:
            ...
            if days_elapsed > 0 and days_elapsed < days_in_month:
                current_spending = abs(
                    current_month_data[current_month_data["amount"] < 0]["amount"].sum()
                )
                ...
                monthly_data = analysis["by_month"]
                if len(monthly_data) >= 2:
                    avg_monthly_spending = np.mean(
                        [abs(data["total_expenses"]) for data in monthly_data.values()]
                    )
    ...
    def _identify_savings_opportunities(self, df, analysis):
        ...
        if len(small_transactions) > 0:
            small_tx_by_category = (
                small_transactions.groupby("category")["amount"]
                .agg(["sum", "count"])
                .reset_index()
            )
    ...
    def _analyze_spending_habits(self, df, analysis):
        ...
        weekend_spending = abs(
            df[(df["amount"] < 0) & (df["is_weekend"])]["amount"].sum()
        )
        weekday_spending = abs(
            df[(df["amount"] < 0) & (~df["is_weekend"])]["amount"].sum()
        )
