...
class DataParser:
            ...
            with pdfplumber.open(file_path) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    for table_num, table in enumerate(tables, 1):
                        ...
                        print(
                            f"Found table {table_num} with {len(table)} rows on page {page_num}"
                        )
                        ...
                        if column_mapping:
                            ...
                            print(
                                f"Extracted {len(transactions)} transactions from table {table_num}"
                            )
    ...
    def _identify_columns(self, df):
        ...
        if "date" not in column_mapping or "description" not in column_mapping:
            print(
                f"Warning: Could not identify required columns. Found: {column_mapping}"
            )
            return None
    ...
    def _parse_dataframe(self, df, column_mapping):
        ...
        for index, row in df.iterrows():
            try:
                transaction = self._parse_row_with_mapping(
                    row, index + 1, column_mapping
                )
                if transaction:
    ...
    def _parse_row_with_mapping(self, row, row_number, column_mapping):
        ...
        description = self._parse_description(
            row[column_mapping["description"]], row_number
        )
        ...
        if "amount" in column_mapping:
            ...
            if debit_amount is not None and debit_amount != 0:
                ...
                raise Exception(
                    f"Row {row_number}: No valid amount in debit or credit columns"
                )
    ...
    def _parse_amount(self, amount_value, row_number):
        ...
        try:
            ...
            raise Exception(
                f"Row {row_number}: Invalid amount '{amount_value}'. Must be a number"
            )
